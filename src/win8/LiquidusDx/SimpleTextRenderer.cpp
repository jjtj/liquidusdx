...
{
	AppResource::ReadTextFile(L"Sample.txt")
		.then([&](Platform::String^ txt) {
			auto obj = JsonObject::Parse(txt);
			auto dim = obj->GetNamedObject(ref new String(L"dim"));
			auto w = dim->GetNamedNumber(ref new String(L"w"));
			auto h = dim->GetNamedNumber(ref new String(L"h"));
	});
}
